...
export function CreateCompanyPage() {
  ...
  const createCompany = useMutation({
    mutationFn: (payload: typeof emptyForm) => api.post<Company>('/companies', payload).then((r) => r.data),
    onSuccess: (company) => {
      setCreated(company);
      setForm(emptyForm);
    ...
    return (
      <div>
        ...
        <p className="mt-2 text-[var(--text-muted)]">
          Essa página é só para o administrador da plataforma ArgramTech, não das empresas individuais.
        </p>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="mt-6 grid max-w-xl gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-sm text-[var(--text-muted)]">Slug (usado na URL: argramtech.com.br/slug)</label>
          <input
            required
            ...
            className="rounded-lg border border-[var(--border)] bg-[var(--bg)] px-3 py-2 font-mono text-sm outline-none focus:border-[var(--brand)]"
          />
        </div>

        <div className="flex flex-col gap-1">
          <label className="text-sm text-[var(--text-muted)]">Cor principal</label>
          <div className="flex items-center gap-2">
            ...
            <input
              value={form.primaryColor}
              onChange={(e) => setForm((f) => ({ ...f, primaryColor: e.target.value }))}
              className="flex-1 rounded-lg border border-[var(--border)] bg-[var(--bg)] px-3 py-2 font-mono text-sm outline-none focus:border-[var(--brand)]"
            />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-sm text-[var(--text-muted)]">Nome do administrador</label>
          <input
            required
        ...
        <div className="flex flex-col gap-1">
          <label className="text-sm text-[var(--text-muted)]">E-mail do administrador</label>
          <input
            required
            type="email"
        ...
        <div className="flex flex-col gap-1">
          <label className="text-sm text-[var(--text-muted)]">Senha do administrador</label>
          <input
            required
            type="password"
            minLength={8}
